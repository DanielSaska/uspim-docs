...
Before compilation, it is important to modify the *Target Platform* and *Platform Toolset*. By default, this is *10.0* and *v143*, respective, as we compiled the project with Visual Stuido 2019. Selecting available target platform and toolset will allow compilation on different operating system versions and versions of the windows C++ compiler. The compilation process is standard from then on. For development, select the Debug configuration, for usage, use the Release configuration.
